...
namespace CloudyShop.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public ActionResult Index(int? page, string SearchString = "")
        {
            ...
            if (SearchString != "")
            {
                var products = db.Products.Where(x => x.Title.ToUpper().Contains(SearchString.ToUpper()));
                return View("Searching", products);
            }
            return View(items);
        }
        ...
        public ActionResult Partial_FeatureProduct()
        {
            var items = db.Products.Where(x => x.IsFeature && x.IsActive).ToList();
            return PartialView(items);
        }
